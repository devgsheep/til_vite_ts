...
const InfiniteTodoList = () => {
  ...
  // 1. IntersectionObserver를 저장하는 ref
  ...
  /**
   * 목록에 마지막 요소를 등록할 겁니다.
   * 목록의 마지막 요소가 화면에 들어오면 isIntersecting을 true로
   * 아직 더 불러올 데이터가 있으면 loadMore를 실행
   * 새로운 목록이 랜더링 되면 새로운 마지막 요소에 다시 옵저버를 붙임
   * 위의 과정을 반복 ==> 끝까지 반복
   */

  // 마지막 todo 항목이 화면에 보이면 자동으로 다음 데이터를 불러들이는 함수
  // 여기서는 useCallback을 사용합니다.
  // - 함수가 리랜더링 될때마다 새롭게 만들면 성능 이슈가 있습니다.
        ...
        <div>
          <ul>
            {todos.map((item, index) => (
              // 마지막 요소 태그인지를 연결한다. (마지막 배열의 index인지 비교하면 됨.)
