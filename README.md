...
- 기본형은 `Table Editor`로 생성하고 추가적 설정 `SQL Editor`
...
## 4. Supabase의 테이블의 칼럼의 데이터 타입 정의
...
- 향후 지시하는 대로 실행함.
- `id`는 URL의 앞쪽 단어가 ID가 됨.
  - id는 Supabase URL의 앞단어
...
```json
  "scripts": {
    ...
    "generate-types": "npx supabase gen types typescript --project-id fmdxkenuulhcysvkybnk --schema public > types_db.ts"
  },
```

- 생성된 ts의 내용을 참조해서 우리가 원하는 곳에 복사 및 붙여넣기
