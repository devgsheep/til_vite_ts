...
- 참고로 실제 프로젝트에서 tsx가 어렵다면, jsx로 작업 후 ai에게 tsx로 만들어줘. 해도 됨.

### ts 프로젝트에서 jsx를 사용하도록 설정
...
- /src/components/TodoWrite.jsx 파일 생성

```jsx
import React, { useState } from 'react';

function TodoWrite({ addTodo }) {
...
- /src/types 폴더 생성
- /src/types/TodoTypes.ts 파일 생성 (확장자 조심)
...
- TodoWrite.tsx 로 변경 (App.tsx에서 다시 import 하고 새로고침)
...
- TodoList.tsx 로 변경 (App.tsx에서 다시 import 하고 새로고침)
...
- TodoItem.tsx 로 변경 (TodoList.tsx에서 다시 import 하고 새로고침)
