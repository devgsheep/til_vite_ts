...
- 참고 자료 : https://www.youtube.com/watch?v=iWQEK8pS2kU
...
- 로그인 후 > 앱 메뉴 > 앱 생성
- 생성된 앱 `나의 할일 서비스` > 앱 > 일반 > 개인 개발자 비즈 앱 전환
- 플랫폼 > Web 플랫폼 등록 > 사이트 등록 > `http://localhost:5173`

### 1.2. 반드시 Rest API 키를 보관해둠.

- `c6bb3fcf43ee80acc41dce8ca4bd05f1`
...
- 카카오 로그인 메뉴 > 동의항목 > 개인정보 > `닉네임, 프로필사진, 카카오계정(이메일)` 활성화

### 1.5. Redirect URI 등록하기

- 카카오 로그인 메뉴 > 일반 > 리다이렉트 URI을 등록함.
- 예) `http://localhost:5173/auth/callback`
- 추후 추가설정 필요

### 1.6. Supabase를 위한 `Client Secert` 설정 필요

- 카카오 로그인 메뉴 > `Client Secert 활성` > 클라이언트 시크릿 발급
- 시크릿 키 보관 : `shRZvF8ZoAYbCLhDOSvmkfSmFJnZ4OOY`
- Callback URL (for OAuth)의 내용을 반드시 보관해 둠.
  `https://fmdxkenuulhcysvkybnk.supabase.co/auth/v1/callback`
- `Callback URL을 카카오 개발자의 Redirect URI에 추가로 저장해 둠.`

## 2. Supabase 인증 세팅하기

- supabase.com > 프로젝트 > Authentication > Sign In / Providers 메뉴
- Auth Providers 항목에서 `Kakao` 활성
- Rest API 키와 시크릿 키를 입력
  ...
  const deleteAccount: AuthContextType['deleteAccount'] = async () => {
    try {
      ...
      if (deleteRequestsError) {
        ...
        return { error: '프로필 탈퇴 목록 추가에 실패했습니다.' };
      }
      ...
      return { error: ' 계절 탈퇴 처리  오류가 발생하였습나다.' };
    }
  };
...
- /src/pages/AuthCallback.tsx 업데이트 (카카오로그인 후 profile 업데이트)
...
function AuthCallback() {
  ...
  // 인증 정보에 담겨진 nickname을 알아내서 여기서 profiles를 추가
  const handleAuthCallback = async (): Promise<void> => {
    try {
      ...
      if (data.session?.user) {
        ...
        // 카카오로 로그인 했는지 확인 필요 (kakao는 Supabase에서 정한 글자)
        const isKakaoLogin = user.app_metadata.provider === 'kakao';

        // 추가적인 정보 파악 가능 (metadata라고 함.)
...
- 이메일 회원은 반드시 이메일 중복 체크 필수
- 닉네임도 반드시 중복 체크 필수
- 회원가입 승인 진행

### 5.1. 이메일과 닉네임 중복은 회원이 아닌 사람이 DB에 접근

- 웹에서 DB로 접근하는 것이 아니고, 바로 DB에 접근하는 코드 필요.
...
### 5.4. 제대로 PostreSQL Function 등록 여부 확인
...
export const AuthProvider: React.FC<PropsWithChildren> = ({ children }) => {
  ...
  // - 좀 위험한 것은 error.message을 문자열로 비교한 것이 좀 불안함.
  ...
  const checkEmailExists: AuthContextType['checkEmailExists'] = async email => {
    ...
    try {
      ...
      if (error) {
        return { exists: false, error: '이메일 확인 중 오류가 발생' };
      }
      return { exists: data.exists };
    } catch (err) {
      console.log('이메일 중복 확인 오류', err);
      return { exists: false, error: '이메일 중복 확인 중 오류가 발생' };
    }
  };
  ...
  const checkNicknameExists: AuthContextType['checkNicknameExists'] = async nickname => {
    ...
    try {
      ...
      if (error) {
        return { exists: false, error: '닉네임 확인 중 오류가 발생' };
      }
      return { exists: data.exists };
    } catch (err) {
      console.log('닉네임 중복 확인 오류', err);
      return { exists: false, error: '닉네임 중복 확인 중 오류가 발생' };
    }
  };
...
- 카카오 로그인 시 인증코드가 제대로 작동안되는 경우 발생
- /src/lib/profile.ts
