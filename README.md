...
- `src/services/chat/dircetChatService.ts 파일 생성`
...
/**
 * Mock 사용자 데이터
 *  실제로는 Supabase의 profiles 테이블에서 가져올 데이터 샘플
 *  고유한 ID, 이메일, 닉네임, 아바타 URL
 */
const mockUser: ChatUser[] = [
  { id: '1', email: 'user1@example.com', nickname: '도도롱', avatar_url: null },
  { id: '2', email: 'user2@example.com', nickname: '도로롱', avatar_url: null },
  { id: '3', email: 'user3@example.com', nickname: '귀요비', avatar_url: null },
  { id: '4', email: 'user4@example.com', nickname: '초판다', avatar_url: null },
];

/**
 * Mock 채팅방 목록 데이터
 *  실제로는 Supabase의 채팅목록 테이블에서 가져올 데이터 샘플
 *  각 목록에는 마지막 메시지, 읽지 않은 메시지 수, 상대방 정보 포함
 */
const mockChats: ChatListItem[] = [
  {
    id: 'chat1',
    name: '도도롱', // 채팅방 이름인데 1:1이라서 상대방 닉네임으로 생성됨
    type: 'direct', // 채팅방 타입
    last_message: {
      // 마지막 메시지 정보(선택사항)
      content: '고급 팰 기름 주세요',
      create_at: new Date(Date.now() - 1000 * 60 * 30).toISOString(),
      sender_nickname: '도로롱',
    },
  ...
  {
    id: 'chat2',
    name: '귀요비', // 채팅방 이름인데 1:1이라서 상대방 닉네임으로 생성됨
    type: 'direct', // 채팅방 타입
    last_message: {
      // 마지막 메시지 정보(선택사항)
      content: '벌꿀 주세요',
...
/**
 * Mock 메시지 데이터
 *  실제로는 Supabase의 메시지 테이블에서 가져올 데이터 샘플
 *  채팅방 ID를 키로 해서 각 채팅방의 메시지를 그룹화후 관리
 */
const mockMessage: { [chatId: string]: MessageDetail[] } = {
  chat1: [
    {
      id: 'msg1',
      chat_id: 'chat1',
...
export async function getMessages(chatId: string): Promise<ChatApiResponse<MessageDetail[]>> {
  // 시뮬레이션으로 백엔드 비동기 느낌으로 0.5초 대기
  await new Promise(resolve => setTimeout(resolve, 500));
  // Mock 데이터로서 해당 채팅방의 메시지들 전체 조회
  const messages = mockMessage[chatId] || null;
...
interface DirectChatContextType {
  ...
  searchUsers: (searchTerm: string) => Promise<void>; // 사용자 검색어로 사용자 검색
...
export const DirectChatProvider: React.FC<DirectChatProviderProps> = ({ children }) => {
  ...
  // 사용자가 선택해서 활성화한 채팅방의 ID를 보관함.
  // 리랜더링이 되어서 값이 갱신되거나, 화면에 보여줄 필욘느 없음
    ...
    async (participantId: string): Promise<string | null> => {
      try {
        ...
        if (response.success && response.data) {
          // 채팅방 새로 고침으로 목록 갱신
          await loadChats();
          return response.data.id; // 새 채팅ID를 전달한 이유는 즉시 채팅방 참여
        } else {
...
export const useDirectChat = () => {
  ...
  if (!context) {
    throw new Error('채팅 컨텍스트가 생성되지 않음');
  }
  return context;
};
```
...
```tsx
function App() {
  return <DirectChatProvider>....</DirectChatProvider>;
}
...
- Context를 통해서 Service API 호출 및 상태 출력
              ...
              <div className="chat-info">
                ...
                <div className="chat-header">
                  ...
                  <div className="chat-time">
                    {formatTime(chat.last_message ? chat.last_message.create_at : '')}
                  </div>
                </div>
                {/* 마지막 메시지 미리보기 */}
