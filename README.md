...
```tsx
// 6. 커스텀 훅
export function useTodos(): InfiniteScrollContextValue {
...
export const InfiniteScrollProvider: React.FC<InfiniteScrollProviderProps> = ({
  ...
  const addTodo = async (title: string): Promise<void> => {
    try {
      ...
      console.log(`새 Todo 등록 실패 : ${error}`);
    }
  };
  ...
  const toggleTodo = async (id: number): Promise<void> => {
    try {
      // 현재 전달 된 id에 해당하는 todo항목의 completed를 파악한다.
...
- 요소 즉, 대상이 되는 태그(element)가
  ...
  - intersect는 DOM 요소가 화면에 보이거나, 사라지거나 등을 말함.
