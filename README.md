...
- 예) http : Hyper Text Transfer Protocol
...
export const supabase = createClient(supabaseUrl, supabaseAnonKey, {
  auth: {
    ...
    // 사용자 세션정보를 localStorage에 저장해서 웹 브라우저 새로고침시에도 로그인 유지
    persistSession: true,
...
const Protected: React.FC<PropsWithChildren> = ({ children }) => {
  const { user } = useAuth();
  // 로그인이 안되어서 user정보가 없으면 로그인페이지로 이동
  if (!user) {
...
- AuthContext에 loading 이라는 처리를 진행해 주고, 활용함
...
- AuthContext.tsx에서 읽어들이기 전까지 loading을 활성화 함.
